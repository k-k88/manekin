...
public function payrolls(Request $request, $companyId)
{
    ...
    foreach ($attendances as $attendance) {
        // --- 時給の決定（WageHistory > attendance.hourly_wage > user.hourly_wage > 1000）
        $wageHistory = WageHistory::where('user_id', $attendance->user_id)
            ->where('effective_from', '<=', $attendance->date)
            ->orderByDesc('effective_from')
            ->first();
        ...
        if ($wageHistory) {
            $hourlyWage = $wageHistory->hourly_wage;
        } elseif (!empty($attendance->hourly_wage)) {
            $hourlyWage = $attendance->hourly_wage;
        } else {
            $hourlyWage = $attendance->user->hourly_wage ?? 1000;
        }
        $attendance->hourly_wage = $hourlyWage;

        // --- 出勤／退勤のパース（必ず date + time を結合）
        try {
        ...
        // 深夜時間帯定義（22:00〜翌05:00）
        $nightStart = Carbon::parse($attendance->date . ' 22:00');
        $nightEnd = Carbon::parse($attendance->date . ' 05:00')->addDay(); // 翌05:00

        // 深夜重なり計算（正しく max/min）
        $nightMinutes = 0;

        // 求めるのは overlap( [clockIn, clockOut), [nightStart, nightEnd) )
        ...
        $totalPay = round(max(0, $normalPay + $nightPay));

        $attendance->hours = round($totalMinutes / 60, 2);
        $attendance->pay = $totalPay;
    }
